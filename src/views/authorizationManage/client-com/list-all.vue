<template>
  <div class="app-container ohn">
    ...
    >
      ...
      <el-table-column prop="lastTraceUser" label="最后跟进人" min-width="150" sortable />
        ...
        <template slot-scope="scope">
          ...
          <el-dropdown>
            ...
            <el-dropdown-menu slot="dropdown">
              <el-dropdown-item @click.native="handleShowTraceHistory(scope.row)">跟进历史</el-dropdown-item>
              <el-dropdown-item @click.native="handleUnlock(scope.row)">放出</el-dropdown-item>
              <el-dropdown-item @click.native="handleTransfer(scope.row)">转移</el-dropdown-item>
            </el-dropdown-menu>
          </el-dropdown>
        </template>
      </el-table-column>
    ...
    <el-dialog :visible.sync="dialogVisible" title="跟进历史">
      <div class="block" style="height: 50vh; overflow-y: scroll">
        <!--        <div class="radio">-->
        <!--          排序：-->
        ...
        <el-timeline :reverse="reverse">
          <el-timeline-item
            ...
            :key="index"
            :timestamp="activity.opDate"
          >
            {{ activity.traceUserName + activity.opType + '，保护期更新至：' + activity.protectDate }}
          </el-timeline-item>
        </el-timeline>
      </div>
    </el-dialog>
    <el-dialog :visible.sync="transferDialogVisible" :title="'转移客户 '+transClient.name">
      <el-form>
        <el-form-item>
          <el-select v-model="selectedUser" placeholder="请选择" style="display: block; width: 240px">
            <el-option
      ...
      <div style="text-align:right;">
        <el-button type="danger" size="small" plain @click="transferDialogVisible=false">取消</el-button>
...
import {
  deleteClientCom,
  getClientTraceHistory,
  queryClientComPageAll, transferClient, unlockClient
...
export default {
  ...
  methods: {
    ...
    fetchData() {
      ...
      queryClientComPageAll(Object.assign({}, queryParam, colParam))
        .then((res) => {
    ...
    handleShowTraceHistory(data) {
      console.log(data)
      getClientTraceHistory({ id: data.id })
        .then(res => {
    ...
    handleTransfer(row) {
      ...
      this.transClient = row
    },
    handleConfirm() {
      // 转移客户
      transferClient(this.transClient, this.selectedUser)
        .then(res => {
          console.log(res.data)
          const { data, success, errorMessage } = res
          if (success) {
