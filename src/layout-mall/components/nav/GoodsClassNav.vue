...
<template>
  <div class="item-class-show">
    <el-row v-for="(items, index) in tagsInfo" :key="index" class="item-class-group">
      <el-col class="item-class-name" :sm="3" :xs="3">{{ items.tagName }} : </el-col>
      <el-col class="item-class-select" :sm="21" :xs="21">
  ...
  data() {
    return {
      tagsInfo: [
        {
          tagName: '品牌',
          tags: ['华为(HUAWEI)', '三星(SAMSUNG)', 'MATE', '摩斯维(msvii)', 'OPPO', '莫凡(Mofi)', '耐尔金(NILLKIN)', '洛克(ROCK)']
        },
        {
          tagName: '手机配件',
