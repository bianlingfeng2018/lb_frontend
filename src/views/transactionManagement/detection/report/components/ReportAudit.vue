<template>
  <div class="app-container ohn">
    ...
    <div v-if="testTradeState==='testReportCreated'">
      <p style="font-family: '仿宋',Arial,sans-serif;">
        编制人 <span style="font-weight: bold;">{{ audit1.nickname }}</span>
      </p>
      <el-button
        type="primary"
        size="small"
...
export default {
  ...
  created() {
    ...
    this.editorQueryParam.tradeId = this.postForm.id
  ...
  methods: {
    fetchData: function(id) {
      queryTestTradeDetail(Object.assign({}, { testTradeId: id })).then(response => {
        ...
        const uid = response.data.testWorkOrder.editorId
        console.log(uid)
        getUserByRoleAndTradeId(this.editorQueryParam)
          .then((res) => {
      ...
      this.$store
        ...
        .then((res) => {
          ...
          if (success) {
            ...
            this.$router.push({
              path: "/tm/detection/report/result",
              query: {
