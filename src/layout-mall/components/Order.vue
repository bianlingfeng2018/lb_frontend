<template>
  <div>
    <Search />
    <!--    <GoodsListNav />-->
    <div class="goods-list-container">
      <el-alert
        class="tips-box"
        title="小提示"
        type="warning"
        description="请点击商品前的选择框，选择购物车中的商品，点击付款即可。"
        show-icon
        :closable="false"
      />
      <!--      <Table ref="selection" border :columns="columns" :data="shoppingCart" size="large" no-data-text="您的购物车没有商品，请先添加商品到购物车再点击购买" @on-selection-change="select" />-->
      <el-table
        ref="selection"
        :data="shoppingCart"
        tooltip-effect="dark"
        style="width: 100%"
        @selection-change="handleSelectionChange"
      >
        <el-table-column
          type="selection"
          width="58"
          align="center"
        />
        <el-table-column
          prop="img"
          label="图片"
          width="86"
        >
          <template slot-scope="scope">
            <div><img height="40vh" width="40vw" :src="getUrl(scope.row.img)" alt="加载失败"></div>
          </template>
        </el-table-column>
        <el-table-column
          prop="title"
          label="标题"
        />
        <el-table-column
          prop="package.items"
          label="套餐"
        >
          <template slot-scope="scope">
            <span v-for="(item, index) in scope.row.package" :key="index">{{ (index + 1) }}.{{ item.testItem }};</span>
          </template>
        </el-table-column>
        <el-table-column
          prop="count"
          label="数量"
        />
        <el-table-column
          prop="price"
          label="价格"
        />
      </el-table>
      <div class="address-container">
        <h3>收货人信息</h3>
        <div class="address-box">
          <div class="address-check">
            <div class="address-check-name">
              <span><el-icon class="el-icon-check" /> {{ checkAddress.name }}</span>
            </div>
            <div class="address-detail">
              <p>{{ checkAddress.address }}</p>
            </div>
          </div>
          <el-collapse>
            <el-collapse-item title="选择地址">
              <el-radio-group v-model="selectedAddress" vertical size="large" @change="changeAddress">
                <el-radio v-for="(item, index) in address" :key="index" :label="item.addressId">
                  <span>{{ item.name }} {{ item.province }} {{ item.city }} {{ item.address }} {{ item.phone }} {{ item.postalcode }}</span>
                </el-radio>
              </el-radio-group>
            </el-collapse-item>
          </el-collapse>
        </div>
      </div>
      <div class="remarks-container">
        <h3>备注</h3>
        <el-input v-model="remarks" size="large" placeholder="在这里填写备注信息" class="remarks-input" />
      </div>
      <div class="invoices-container">
        <h3>发票信息</h3>
        <p>该商品不支持开发票</p>
      </div>
      <div class="pay-container">
        <div class="pay-box">
          <p><span>提交订单应付总额：</span> <span class="money">￥{{ totalPrice.toFixed(2) }}</span></p>
          <div class="pay-btn">
            <router-link to="/mall/order-done"><el-button type="danger" size="large">提交订单</el-button></router-link>
          </div>
        </div>
      </div>
    </div>
  </div>
</template>

<script>
import Search from '@/layout-mall/components/Search'
// import GoodsListNav from '@/layout-mall/components/nav/GoodsListNav'
import store from '@/layout-mall/vuex/store'
import { mapState, mapActions } from 'vuex'
import { getFileBlobUrl } from "@/utils/simple-util"
export default {
  name: 'Order',
  beforeRouteEnter(to, from, next) {
    window.scrollTo(0, 0)
    next()
  },
  data() {
    return {
      goodsCheckList: [],
      columns: [
        {
          type: 'selection',
          width: 58,
          align: 'center'
        },
        {
          title: '图片',
          key: 'img',
          width: 86,
          render: (h, params) => {
            return h('div', [
              h('img', {
                attrs: {
                  src: params.row.img
                }
              })
            ])
          },
          align: 'center'
        },
        {
          title: '标题',
          key: 'title',
          align: 'center'
        },
        {
          title: '套餐',
          width: 198,
          key: 'package',
          align: 'center'
        },
        {
          title: '数量',
          key: 'count',
          width: 68,
          align: 'center'
        },
        {
          title: '价格',
          width: 68,
          key: 'price',
          align: 'center'
        }
      ],
      checkAddress: {
        name: '未选择',
        address: '请选择地址'
      },
      remarks: '',
      multipleSelection: [],
      selectedAddress: ''
    }
  },
  created() {
    this.loadAddress()
    console.log(this.shoppingCart)
  },
  computed: {
    ...mapState(['address', 'shoppingCart']),
    totalPrice() {
      let price = 0
      this.goodsCheckList.forEach(item => {
        price += item.price * item.count
      })
      return price
    }
  },
  methods: {
    ...mapActions(['loadAddress']),
    getUrl(file) {
      return getFileBlobUrl(file)
    },
    select(selection, row) {
      console.log(selection)
      this.goodsCheckList = selection
    },
    changeAddress(data) {
      const father = this
      this.address.forEach(item => {
        if (item.addressId === data) {
          father.checkAddress.name = item.name
          father.checkAddress.address = `${item.name} ${item.province} ${item.city} ${item.address} ${item.phone} ${item.postalcode}`
        }
      })
    },
    toggleSelection(rows) {
      if (rows) {
        rows.forEach(row => {
          this.$refs.selection.toggleRowSelection(row)
        })
      } else {
        this.$refs.selection.clearSelection()
      }
    },
    handleSelectionChange(val) {
      this.multipleSelection = val
    }
  },
  mounted() {
    setTimeout(() => {
      // this.$refs.selection.selectAll(true)
      this.$refs.selection.toggleAllSelection()
    }, 500)
  },
  components: {
    Search
    // GoodsListNav
  },
  store
}
</script>

<style scoped>
.goods-list-container {
  margin: 15px auto;
  width: 80%;
}
.tips-box {
  margin-bottom: 15px;
}
.address-container {
  margin-top: 15px;
}
.address-box {
  margin-top: 15px;
  padding: 15px;
  border: 1px #ccc dotted;
}
.address-check {
  margin: 15px 0px;
  height: 36px;
  display: flex;
  align-items: center;
}
.address-check-name {
  width: 120px;
  display: flex;
  justify-content: center;
  align-content: center;
  background-color: #ccc;
}
.address-check-name span {
  width: 120px;
  height: 36px;
  font-size: 14px;
  line-height: 36px;
  text-align: center;
  color: #fff;
  background-color: #f90013;
}
.address-detail {
  padding-left: 15px;
  font-size: 14px;
  color: #999999;
}
.remarks-container {
  margin: 15px 0px;
}
.remarks-input {
  margin-top: 15px;
}
.invoices-container p{
  font-size: 12px;
  line-height: 30px;
  color: #999;
}
.pay-container {
  margin: 15px;
  display: flex;
  justify-content: flex-end;
}
.pay-box {
  font-size: 18px;
  font-weight: bolder;
  color: #495060;
}
.money {
  font-size: 26px;
  color: #f90013;
}
.pay-btn {
  margin: 15px 0px;
  display: flex;
  justify-content: flex-end;
}
</style>
